...
#pragma pack(push, 4)
	...
	// Trap-frame continuation routines: load the volatile registers from
	// the trap-frame at `rbp - 0x80` and `iretq` / `sysret`.  Both are
	// MSVC-style naked-asm functions; only the LLVM-MSVC fork (the
	// patched clang ntbind targets) accepts the syntax used here.  Vanilla
	// clang chokes on the `using` line and the assembler dialect, so
	// gate them behind `NTBIND_KERNEL_ASM` -- consumers that need the
	// continuation routines define the macro at their build's site.
	//
#ifdef NTBIND_KERNEL_ASM
	// @ rbp = tf - 0x80
